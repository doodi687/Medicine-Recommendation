...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        ...
        if item == "" or item not in symptoms_dict:
            # invalid symptom → directly show error
            return None
...
def home():
    ...
    if symptoms == "":
        return render_template("index.html",
                               message="⚠ Please enter symptoms before prediction.")

    # Convert comma-separated input to list
    user_symptoms = [s.strip().lower() for s in symptoms.split(',')]

    # ---------------- FIX: Remove empty items from list ----------------
    user_symptoms = [sym for sym in user_symptoms if sym != ""]

    if len(user_symptoms) == 0:
        return render_template("index.html",
                               message="⚠ Please enter valid symptoms.")

    # ---------------- FIX: Validate each symptom ----------------
    for s in user_symptoms:
        if s not in symptoms_dict:
            return render_template("index.html",
                                   message=f"⚠ '{s}' is not a valid symptom. Check spelling.")
    ...
    if predicted_disease is None:
        return render_template("index.html",
                               message="⚠ Invalid or misspelled symptoms.")

    dis_des, precautions_list, med_list, diet_list, workout_list = helper(predicted_disease)
